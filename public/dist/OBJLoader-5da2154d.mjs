import { C as N, L as R, F as H, G as T, p as P, q as w, h as C, K as U, k as V, n as k, W as D, l as F, j as G, a3 as L } from "./index-20e94914.mjs";
const q = /^[og]\s*(.+)?/, J = /^mtllib /, W = /^usemtl /, K = /^usemap /, B = /\s+/, E = new L(), z = new L(), O = new L(), S = new L(), j = new L(), _ = new N();
function Q() {
  const I = {
    objects: [],
    object: {},
    ...
    addFaceNormal: function(s, e, n) {
      ...
      E.fromArray(t, s), z.fromArray(t, e), O.fromArray(t, n), j.subVectors(O, z), S.subVectors(E, z), j.cross(S), j.normalize(), i.push(j.x, j.y, j.z), i.push(j.x, j.y, j.z), i.push(j.x, j.y, j.z);
    },
...
class Y extends R {
  ...
  load(s, e, n, t) {
    const i = this, a = new H(this.manager);
          ...
          switch (o[0]) {
            case "v":
              e.vertices.push(
                ...
                parseFloat(o[5]),
                parseFloat(o[6])
              ).convertSRGBToLinear(), e.colors.push(_.r, _.g, _.b)) : e.colors.push(void 0, void 0, void 0);
              break;
            case "vn":
              e.normals.push(
                parseFloat(o[1]),
      ...
      for (let d = 0, g = e.objects.length; d < g; d++) {
        ...
        for (let v = 0, M = o.length; v < M; v++) {
          ...
          if (this.materials !== null) {
            ...
            } else if (u && f && !(f instanceof V)) {
              const x = new V({ size: 10, sizeAttenuation: !1 });
              U.prototype.copy.call(x, f), x.color.copy(f.color), x.map = f.map, f = x;
            }
          }
          f === void 0 && (l ? f = new C() : u ? f = new V({ size: 1, sizeAttenuation: !1 }) : f = new k(), f.name = y.name, f.flatShading = !y.smooth, f.vertexColors = m, e.materials[A] = f), p.push(f);
        }
        let b;
        if (p.length > 1) {
          for (let v = 0, M = o.length; v < M; v++) {
          ...
          l ? b = new D(c, p) : u ? b = new F(c, p) : b = new G(c, p);
        } else
          l ? b = new D(c, p[0]) : u ? b = new F(c, p[0]) : b = new G(c, p[0]);
        b.name = r.name, i.add(b);
      }
    else if (e.vertices.length > 0) {
      const d = new V({ size: 1, sizeAttenuation: !1 }), g = new P();
